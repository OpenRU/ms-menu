...
class MenuPutSchema(ma.SQLAlchemySchema):
    ...
    date = ma.Date()
    main_dish = ma.String(validate=validate.Length(min=1, max=255))
    side_dish = ma.String(validate=validate.Length(min=1, max=255))
    salad = ma.String(validate=validate.Length(min=1, max=255))
    dessert = ma.String(validate=validate.Length(min=1, max=255))
    drink = ma.String(validate=validate.Length(min=1, max=255))
